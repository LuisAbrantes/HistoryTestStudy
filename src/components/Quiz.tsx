...
const Quiz = ({ topicId }: QuizProps) => {
    ...
    // Removed selectedOption state as it is unused
                    ...
                    <div className="space-y-3 mb-6">
                        {questions[currentQuestion].options.map(
                            (option, index) => {
                                ...
                                let optionClass =
                                    'border border-gray-300 rounded-md p-3 cursor-pointer hover:bg-gray-50';

                                if (isAnswered) {
                                    if (isSelected && isCorrect) {
                                        optionClass =
                                            'border-2 border-green-500 bg-green-50 rounded-md p-3';
                                    } else if (isSelected && !isCorrect) {
                                        optionClass =
                                            'border-2 border-red-500 bg-red-50 rounded-md p-3';
                                    } else if (isCorrect) {
                                        optionClass =
                                            'border-2 border-green-500 bg-green-50 rounded-md p-3';
                                    } else {
                                        optionClass =
                                            'border border-gray-300 rounded-md p-3 opacity-60';
                                    }
                                }
